...
- [spfa 算法（队列优化的Bellman-Ford算法）](#spfa-算法队列优化的bellman-ford算法)
...
bool spfa()
{
    // 不需要初始化dist数组. 只是求负环, 不一定要求最短路
    // 原理：如果某条最短路径上有n个点（除了自己），那么加上自己之后一共有n+1个点，由抽屉原理一定有两个点相同，所以存在环。

    queue<int> q;
    for (int i = 1; i <= n; i ++ )
    {//负环不一定是从1开始的路径, 所以将所有点都加入
        q.push(i);
        st[i] = true;
    }
...
//题目如果存在负权边, 则实际最短路的值会比无穷大小
//所以判断为if(d[a][b] > INF / 2) cout << "impossible";
void floyd()
{
    for (int k = 1; k <= n; k ++ )
        for (int i = 1; i <= n; i ++ )
